"""
单独为药典三部和四部生成 QA 对，复用 medical_pdf_to_qa_pipeline.py 的 LLM 配置。
生成结果保存为独立的 cache 文件。
"""
...
TARGET_BOOKS = [
    {
        "book_name": "中华人民共和国药典2020年版三部",
        "text_path": str(MARKDOWN_DIR / "中华人民共和国药典2020年版三部" / "auto" / "中华人民共和国药典2020年版三部.md"),
        "source": "中华人民共和国药典2020年版三部",
    },
    {
        "book_name": "中华人民共和国药典2020年版四部",
        "text_path": str(MARKDOWN_DIR / "中华人民共和国药典2020年版四部" / "auto" / "中华人民共和国药典2020年版四部.md"),
        "source": "中华人民共和国药典2020年版四部",
    },
]
...
def main():
    ...
    for book in TARGET_BOOKS:
        ...
        # 构建输入 manifest
        input_path = CACHE_DIR / f"ypd34_input_{book_name}.jsonl"
        ...
        # 分块
        chunk_cache = CACHE_DIR / f"ypd34_chunks_{book_name}.jsonl"
        if not chunk_cache.exists():
            ...
            storage_chunk = FileStorage(
                ...
                file_name_prefix=f"ypd34_chunk_{book_name}",
                cache_type="jsonl",
            )
            ...
            chunk_result = CACHE_DIR / f"ypd34_chunk_{book_name}_step3.jsonl"
    ...
    if all_qa_rows:
        ...
        output_path = CACHE_DIR / "ypd34_qa_pairs.jsonl"
        df_all.to_json(str(output_path), orient="records", lines=True, force_ascii=False)
        print(f"\n=== 完成 ===")
        print(f"三部 + 四部共生成 {len(df_all)} 个 QA 对")
