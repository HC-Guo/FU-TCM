"""
《中西医结合望诊启迪》第7章 唇周异常 VQA 生成脚本。

核心机制：雷达扫描裂变（Radar-Scan-Fission）
  - 读取 context_text，识别命中部位
  - 候选池：唇周/口周、鼻部、舌体、眼部、整体面色
  - 对每个命中部位独立生成 5 题 CoT 题组
  - Q4 专属：胃镜/Holter/MMSE/HP 等内镜与心脑评估指标
  - Q3 补充：药源性诱因识别

输入：
  maizhen_vqa_workdir/output/manual_split/中西医结合望诊启迪/split_samples.jsonl
  （仅处理 figure_id 前缀为 "7-" 的条目）

输出：
  maizhen_vqa_workdir/output/zhongxi_vqa/ch7/vqa_dataset.jsonl
  maizhen_vqa_workdir/output/zhongxi_vqa/ch7/raw_debug.txt
"""
...
DEFAULT_OUTPUT = WORK_DIR / "output" / "zhongxi_vqa" / "ch7"
BOOK_NAME = "中西医结合望诊启迪"
CHAPTER = 7

# ── 第7章 系统提示词（内镜与心脑验证版）────────────────────────────────────
ZHONGXI_CH7_PROMPT = """你是一名中西医结合临床望诊专家，专注于唇周及口周异常与消化系统、心脑血管系统病变的关联判断。

【输入格式】
每条样本包含以下字段：
- `image_caption`：图片编号（如"图7-3"）
- `section_title`：病例标题
...
5. 若无明确命中，则生成一套以"唇周/口周"为部位的默认题组
...
Q3（clinical_diagnosis）：
  ...
  答案：给出"诊断"字段的完整结论
  补充规则：若 context_text 中提及"药物史"、"长期服药"或"既往用药"，作答时应判断诊断是否与药源性因素相关

Q4（gold_standard_labs）：
  问题：辅助检查中使用了哪些客观评估手段？包括内镜、心电、神经评分或生化指标，结果如何？
  答案：精确提取以下信息（如有，含数值和单位）：
    - 胃镜：萎缩分级（轻/中/重）、糜烂、溃疡 Stahl 分期（A1/A2/H1/H2/S1/S2）、HP 检测结果（+/-/++/+++）
    - 肠镜：慢性结肠炎、息肉分型（山田分型 I-IV 型）
    - 心脏评估：心脏彩超 EF 值（%）、Holter 心律失常类型（室早/房早/阵发性房颤等）
    - 神经评估：MMSE 评分（简易精神状态量表，满分30分）、神经系统阳性体征
    - 病理报告：黏膜活检病理分级（慢性炎症程度、肠化、不典型增生）
    - 生化指标：ALT（U/L）、血脂、血糖等

Q5（chain_of_thought_mapping）：
  问题：医生是如何将[命中部位]的空间位置与最终诊断联系起来的？
  答案：引用"分析"字段，说明口周/唇部体征区域与消化系统、心脑血管系统的中医投射对应关系；
...
[
  [
    ...
    },
    {
      ...
      "answer": "（诊断结论，含药源性因素评估）",
      "type": "clinical_diagnosis",
      "hit_region": "[命中部位]"
    },
    {
      "question": "辅助检查中使用了哪些客观评估手段？包括内镜、心电、神经评分或生化指标，结果如何？",
      "answer": "（所有检查项目和结果，含数值和单位）",
...
- MMSE 评分、EF 值、Stahl 分期等数值必须与原文完全一致
...
# ── 核心处理器 ────────────────────────────────────────────────────────────────
class ZhongxiCh7VQAProcessor(ShezhenVQAProcessor):
    """《中西医结合望诊启迪》第7章 唇周异常 VQA 处理器（内镜与心脑验证版）。"""
    ...
    def _load_samples(
        ...
        """加载 split_samples.jsonl，仅保留第7章样本（figure_id 前缀为 '7-'）。"""
        import random
    ...
    def run(
        ...
        conversations = [
            [
                {"role": "system", "content": ZHONGXI_CH7_PROMPT},
                {"role": "user", "content": u},
            ]
        ...
        with debug_file.open("w", encoding="utf-8") as dbg, out_file.open(
            ...
            for idx, (sample, resp) in enumerate(
                ...
                for group in qa_groups:
                    ...
                    hit_region = str(group[0].get("hit_region", "唇周/口周")).strip()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=f"《中西医结合望诊启迪》第{CHAPTER}章 唇周异常 雷达扫描裂变 VQA 生成脚本"
    )
    parser.add_argument(
        "--input-dir",
        type=str,
    ...
    processor = ZhongxiCh7VQAProcessor(
        input_dir=input_dir,
        output_dir=output_dir,
