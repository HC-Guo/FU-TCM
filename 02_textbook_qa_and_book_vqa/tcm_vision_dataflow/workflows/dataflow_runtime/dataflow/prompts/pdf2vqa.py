...
# 全书大致结构（《舌诊快速入门》），供模型对齐章节层级，勿凭此编造正文
_TONGUE_BOOK_OUTLINE = """
Book outline (use only to recognize hierarchy, not to invent text):
- 第一章：舌诊须知（本任务整体跳过，不生成 QA）
- 第二章：舌象所反映的疾病（舌质/舌苔/舌脉/舌觉）
- 第三章：常见病舌诊（呼吸、消化、肝胆、心脑血管、内分泌、泌尿生殖、骨关节等）
- 目录、版权、页眉页脚、纯页码、跨页残句等无效内容不生成
- 图文配对遵循同页就近与 id 邻近原则，避免跨页借证据
"""
...
class QAExtractPrompt(PromptABC):
    ...
    def build_prompt(self) -> str:
        PROMPT = f"""
你是《舌诊快速入门》的**VQA 数据生成**编辑。输入为**一个 JSON 数组**：每项有整数 **id**（按顺序编号）及正文字段；包含图片项（`img_path`、图注/说明文字等）。问答须紧扣中文原文与图注，输出全部为中文。
...
**核心任务**
- **第一章整体跳过**：凡属于第一章（舌诊须知/基础知识）的内容，一律不生成 QA。
- **仅第二章、第三章可生成**，并且按章节严格限制 QA 类型：
  - **第二章（舌象所反映的疾病）**：只允许两类
    1) **有什么特征类**（图中舌象有哪些特征）
    2) **提示病症或由什么导致类（二选一）**：必须根据原文语义择一，若原文更偏“提示何病”就问病症；若更偏“由何导致”就问成因，禁止混问。
  - **第三章（常见病舌诊）**：默认两类，且满足严格条件时可增加第三类
    1) **有什么特征类**
    2) **提示什么类**（提示什么病症/病证/异常）
    3) **如何治疗类（条件触发）**：仅当同时满足以下全部条件才允许生成  
       - 原文明确出现治疗信息（治法/方药/针灸/灸法/贴敷/食疗/明确步骤）；  
       - 治疗方法表述具体明确，不是泛泛描述；  
       - 能确认该治疗信息与当前图片对应（同段图号、同图题注或同页紧邻且语义唯一）。  
       - 任一条件不满足，禁止生成“如何治疗类”。
- 除上述允许类别外，**禁止生成任何其他类型 QA**（如预后、护理、鉴别、机制扩写等）。
- 某一类型若证据不足可不生成；有证据才生成，不得臆造。
- 章节判定优先：标题文本 > 就近小节 > 页码线索。
- 每一类 QA 参考以下**典型示例问句**。生成时可改写措辞，但语义必须与对应类别完全一致：
  - **第二章-特征类（示例）**
    - `图中舌象有什么特征？`
    - `该图可见的舌象特征有哪些？`
  - **第二章-病症提示类（示例）**
  ...
  - **第三章-提示什么类（示例）**
    - `该图舌象提示什么？`
    - `图中舌象可提示哪类病症？`
  - **第三章-如何治疗类（仅在满足触发条件时可用，示例）**
    - `该图对应证候如何治疗？`
    - `针对该图舌象提示的病证，可采用什么治疗方法？`
...
**图片处理规则（最重要）**
1. 每个有效 `img_path` 优先生成 2 条 `<qa_pair>`（对应当前章节允许的两类）；若某一类证据不足可只生成另一类，不得臆造。
2. 每条 `<qa_pair>` 内恰好一个 `<pic>`，且 `<pic>` 必须与该条问题语义主体一致。
2.1 `<pic>` 的值必须**逐字复制**输入 JSON 中对应图片的原始 `img_path`（大小写、扩展名、目录层级完全一致）。
2.2 严禁对图片路径做任何改写：不得截断、补全、猜测、哈希改写、替换目录、替换后缀。
2.3 严禁输出占位符或伪路径：如 `...`、`xxx....jpg`、`images/xxx.jpg`、`unknown.jpg`、空路径等。
3. 图文配对优先级（本书定制）：
   - 只允许使用与该 `img_path` 同一 `page_idx` 的文本作为核心证据。
   - 同页内按 `id` 就近原则取证：优先图下说明、同段文字、前后邻近句。
   - 第二章可适度放宽为前后邻近句用于补充“病症或成因”证据；第三章优先使用图注与最近邻句。
   - 对“如何治疗类”必须额外满足“治疗文本明确且与该图可唯一对应”，否则跳过治疗类。
...
6. 第三章中若图片为穴位图、经络定位图、针灸/艾灸操作示意图、人体部位治疗示意图（即非舌象照片/舌体局部图），一律跳过，不生成 QA。

**`<empty></empty>` 触发条件（严格）**
-- 只有当以下条件**同时满足**时，才允许输出 `<empty></empty>`：
  1) 本段 JSON 中不存在可用 `img_path`；
  2) 不存在可抽取的中文证据文本（同页图注/同页邻近说明）；
  3) 内容属于第一章基础知识或其他明显无关内容（目录、版权、页眉页脚等）。
...
4. 禁止生成与舌诊主题无关图片的 QA。
5. 第二章仅允许“特征 +（病症提示或成因二选一）”；第三章允许“特征 + 提示什么”，并仅在满足严格条件时增加“如何治疗”。
6. 禁止输出未被允许的其他类别（如预后、护理、鉴别等）。
7. 第一章内容一律不生成；仅第二章与第三章按可见证据生成。
...
<qa_pair><label>1</label><question>图中舌象有哪些可见特征？</question><answer>可见舌质淡白，舌体偏胖嫩，苔白腻。</answer><solution>依据同页图注与邻近说明，该图主要表现为舌质淡白、舌体胖嫩及白腻苔。</solution><source_text>id:xxx 同页图注；id:yyy 同页邻近句原文</source_text><pic>（这里必须逐字填写输入 JSON 的真实 img_path）</pic></qa_pair>
<qa_pair><label>2</label><question>该图舌象提示什么主病？</question><answer>提示脾阳不足、痰湿内停相关病证。</answer><solution>依据同页原文，该舌象与脾虚湿盛或痰湿内阻等主病线索相关。</solution><source_text>id:yyy 同页对应原文</source_text><pic>（这里必须逐字填写输入 JSON 的真实 img_path）</pic></qa_pair>
</chapter>
...
Reminder: skip chapter 1 entirely; generate only for chapter 2 and chapter 3.
Reminder: chapter-2 categories are 特征 + (病症提示 or 成因, choose one by source semantics).
Reminder: chapter-3 categories are 特征 + 提示什么, and may include 如何治疗 only when treatment text is explicit, specific, and uniquely tied to the same image.
Reminder: do page-level matching only; never use cross-page evidence for any image.
Reminder: `<pic>` must copy the exact `img_path` from input JSON; never output placeholders like `...` or `xxx....jpg`.
"""
        return PROMPT
