"""
《中西医结合望诊启迪》第1章 面部分区与反射区理论图 VQA 生成脚本。

核心机制：理论图解问答（Theory-Diagram-QA）
  - 第1章为面部望诊理论基础，图片为面部分区图、反射区图、面色图等示意图
  - 非病例图，无"主诉/诊断/辅助检查"结构
  - 生成围绕"图示内容理解"和"中医理论应用"的 5 题组

输入：
  maizhen_vqa_workdir/output/manual_split/中西医结合望诊启迪/split_samples.jsonl
  （仅处理 figure_id 前缀为 "1-" 的条目）

输出（临时中间文件）：
  maizhen_vqa_workdir/output/zhongxi_vqa/ch1/vqa_dataset.jsonl
  maizhen_vqa_workdir/output/zhongxi_vqa/ch1/raw_debug.txt
...
DEFAULT_OUTPUT = WORK_DIR / "output" / "zhongxi_vqa" / "ch1"
BOOK_NAME = "中西医结合望诊启迪"
CHAPTER = 1

# ── 第1章 系统提示词（理论图解版）────────────────────────────────────────────
ZHONGXI_CH1_PROMPT = """你是一名中西医结合临床望诊专家，擅长解读面部望诊理论图示，并将图示内容转化为教学问答。

【输入格式】
每条样本包含以下字段：
- `image_caption`：图片编号（如"图1-3"）
- `section_title`：图片标题（如"面部脏腑分区图"）
- `context_text`：图片对应的理论说明文字

【任务说明】
本章图片为面部望诊理论示意图（分区图、反射区图、面色图等），非临床病例图。
请根据 context_text 的理论内容，生成一套 5 题教学问答组。

【每套题组结构】
每套题组为一个 JSON 数组，包含 5 个题目对象：

Q1（diagram_recognition）：
  问题：该图展示的是什么内容？图中标注了哪些主要区域或要素？
  答案：根据 section_title 和 context_text 描述图示的核心内容

Q2（theory_principle）：
  问题：该图所体现的中医望诊理论依据是什么？
  答案：从 context_text 中提取中医理论基础（如脏腑对应、经络循行、全息理论等）

Q3（region_mapping）：
  问题：根据图示，面部某一具体区域（如额部/鼻部/颧部/颏部）对应哪些脏腑或病理信息？
  答案：从 context_text 中提取具体的面部区域与脏腑/病理的对应关系

Q4（clinical_application）：
  问题：临床上如何利用该图示指导望诊实践？
  答案：从 context_text 中提取临床应用方法或注意事项

Q5（comparison_or_extension）：
  问题：该图示与中医传统望诊理论（如《内经》面部分候）有何联系或发展？
  答案：从 context_text 中提取理论渊源或创新点；若文本未提及，则注明"文本未提供相关说明"
...
[
  {
    "question": "该图展示的是什么内容？图中标注了哪些主要区域或要素？",
    "answer": "（根据context_text描述）",
    "type": "diagram_recognition",
    "hit_region": "整体面部分区"
  },
  {
    "question": "该图所体现的中医望诊理论依据是什么？",
    "answer": "（中医理论基础）",
    "type": "theory_principle",
    "hit_region": "整体面部分区"
  },
  {
    "question": "根据图示，面部某一具体区域对应哪些脏腑或病理信息？",
    "answer": "（具体区域与脏腑对应关系）",
    "type": "region_mapping",
    "hit_region": "整体面部分区"
  },
  {
    "question": "临床上如何利用该图示指导望诊实践？",
    "answer": "（临床应用方法）",
    "type": "clinical_application",
    "hit_region": "整体面部分区"
  },
  {
    "question": "该图示与中医传统望诊理论有何联系或发展？",
    "answer": "（理论渊源或创新点）",
    "type": "comparison_or_extension",
    "hit_region": "整体面部分区"
  }
]
...
- Q3 中的"某一具体区域"应从 context_text 中选取最具代表性的区域
...
# ── 核心处理器 ────────────────────────────────────────────────────────────────
class ZhongxiCh1VQAProcessor(ShezhenVQAProcessor):
    """《中西医结合望诊启迪》第1章 面部分区理论图 VQA 处理器。"""
    ...
    def _load_samples(
        ...
        """加载 split_samples.jsonl，仅保留第1章样本（figure_id 前缀为 '1-'）。"""
        import random
    ...
    def run(
        ...
        conversations = [
            [
                {"role": "system", "content": ZHONGXI_CH1_PROMPT},
                {"role": "user", "content": u},
            ]
        ...
        with debug_file.open("w", encoding="utf-8") as dbg, out_file.open(
            ...
            for idx, (sample, resp) in enumerate(
                ...
                for qa in qa_list:
                    ...
                    hit_region = str(qa.get("hit_region", "整体面部分区")).strip()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=f"《中西医结合望诊启迪》第{CHAPTER}章 面部分区理论图 VQA 生成脚本"
    )
    parser.add_argument(
        "--input-dir",
        type=str,
    ...
    processor = ZhongxiCh1VQAProcessor(
