...
MODEL_NAME = os.getenv("TCM_MODEL_NAME") or "kimi-k2-thinking"
# 可选配置
MAX_TOKENS = 2048
TEMPERATURE = 0.3  # 较低温度保证答案忠实于原文
CONCURRENT_REQUESTS = 5  # 并发数（根据API限制调整）
...
def main():
    """主函数 - 批量处理01_herbal/format_xml目录下所有文件"""

    # 目录配置
    INPUT_DIR = PROJECT_ROOT / "中医古籍分类/01_herbal/format_xml"
    CHECKPOINT_DIR = PROJECT_ROOT / "qa_output/01_herbal"         # 旧输出，用于读断点
    OUTPUT_DIR = PROJECT_ROOT / "qa_output_v2/01_herbal"          # 新输出
    ...
    for i, input_file in enumerate(txt_files, 1):
        # 输出文件名保持序号一致，如 000.txt -> 000_qa.json
