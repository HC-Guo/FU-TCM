...
**核心任务**
- **第一章整体跳过**：凡属于第一章（舌诊辨证基础知识）的内容，一律不生成 QA。
- 当前任务为“第一部分分卷”生成（原书第41-218页已切分为独立 PDF），**不得再按当前文件页码做 <41 过滤**，仅按章节标题与正文语义判断是否属于第二章/第三章。
- **Part1 内容边界（强约束）**：当前分卷仅覆盖“第二章 + 第三章第1-6节”。若识别到第三章第7节及之后（如第七节神经系统疾病、第八节泌尿系统疾病、第九节妇科疾病、第十节男科疾病、第十一节运动系统疾病等），一律不生成。
- 从第二章内容开始生成，并严格按章节类别限制 QA 类型：
...
- 只有当以下条件**同时满足**时，才允许输出 `<empty></empty>`：
  1) 本段 JSON 中不存在可用的舌诊相关 `img_path`；
  2) 不存在可抽取的舌诊中文证据文本（图注/图号句/邻近说明）；
  3) 不属于第二章或第三章第1-6节可生成范围。
- 只要本段存在任一可用舌诊 `img_path` 且可找到证据，必须输出 `<chapter>...</chapter>`，禁止输出 `<empty></empty>`。
...
7. 第一章内容一律不生成；分卷文件内页码仅作顺序参考，不作为章节起始判定条件。
...
Reminder: skip Chapter 1 entirely. This run uses the split Part-1 PDF (original pages 41-218), so classify by chapter text instead of local page numbers.
"""
        return PROMPT
