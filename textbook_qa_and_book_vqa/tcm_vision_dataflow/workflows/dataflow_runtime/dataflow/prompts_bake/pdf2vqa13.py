...
# 全书大致结构（《望舌诊病》），供模型对齐章节层级，勿凭此编造正文
_TONGUE_BOOK_OUTLINE = """
Book outline (use only to recognize hierarchy, not to invent text):
- 第一章：望舌诊病基本知识（本任务整体跳过，不生成 QA）
- 第二章：舌质/舌苔所反映的疾病（只允许两类 QA：特征；病症提示或成因）
- 第三章：望舌诊疗常见病（只允许两类 QA：特征；提示什么）
...
class QAExtractPrompt(PromptABC):
    ...
    def build_prompt(self) -> str:
        PROMPT = f"""
你是《望舌诊病》的**VQA 数据生成**编辑。输入为**一个 JSON 数组**：每项有整数 **id**（按顺序编号）及正文字段；包含图片项（`img_path`、图注/说明文字等）。问答须紧扣中文原文与图注，输出全部为中文。
...
Reminder: chapter-2 categories are 特征 + (病症提示 or 成因, choose one by source semantics); chapter-3 categories are 特征 + 提示什么.
