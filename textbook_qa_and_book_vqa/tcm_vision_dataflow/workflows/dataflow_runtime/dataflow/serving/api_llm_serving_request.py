    ...
    def _api_chat_with_id(
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    return id, None
            # ✅ 1) 连接阶段超时：记录并交给外层重试，不直接打断整批任务
            except requests.exceptions.ConnectTimeout as e:
                cost = time.time() - start
                warnings.warn(f"API connect timeout (id={id}) cost={cost:.2f}s: {e}", RuntimeWarning)
                return id, None
            ...
            # ✅ 4) ConnectionError：这里面 Win/Linux/urllib3 可能包装了各种“超时/断开”
                ...
                # 其它连接错误：refused/reset/remote disconnected 等，常见于瞬时网络抖动
                warnings.warn(f"API connection error (id={id}) cost={cost:.2f}s: {e}", RuntimeWarning)
                return id, None
    ...
    def _api_chat_id_retry(self, id, payload, model, is_embedding : bool = False, json_schema: dict = None):
        for i in range(self.max_retries):
            try:
                ...
                # 兜底：即使底层未来恢复抛异常，也不要让线程直接崩掉
                self.logger.warning(f"Runtime error during API call (id={id}, try={i+1}/{self.max_retries}): {e}")
                response = None
            if response is not None:
                return id, response
            # 指数退避 + 轻微抖动，降低并发重试导致的雪崩
            sleep_s = (2 ** i) + ((id % 7) * 0.1)
            time.sleep(sleep_s)
        return id, None    
