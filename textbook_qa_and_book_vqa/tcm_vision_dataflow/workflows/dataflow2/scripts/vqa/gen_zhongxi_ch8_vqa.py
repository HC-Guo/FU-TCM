"""
《中西医结合望诊启迪》第8章 眼部异常 VQA 生成脚本。

核心机制：雷达扫描裂变（Radar-Scan-Fission）
  - 读取 context_text，识别命中部位
  - 候选池：眼周/眼睑、白睛/黑睛、舌体、整体面容/面神经
  - 对每个命中部位独立生成 5 题 CoT 题组
  - Q4 专属：HP(+/++/+++)微生物检测、胃镜分级、腹部超声、面神经评估

输入：
  maizhen_vqa_workdir/output/manual_split/中西医结合望诊启迪/split_samples.jsonl
  （仅处理 figure_id 前缀为 "8-" 的条目）

输出：
  maizhen_vqa_workdir/output/zhongxi_vqa/ch8/vqa_dataset.jsonl
  maizhen_vqa_workdir/output/zhongxi_vqa/ch8/raw_debug.txt
"""
...
DEFAULT_OUTPUT = WORK_DIR / "output" / "zhongxi_vqa" / "ch8"
BOOK_NAME = "中西医结合望诊启迪"
CHAPTER = 8

# ── 第8章 系统提示词（微生物/病原体扩容版）──────────────────────────────────
ZHONGXI_CH8_PROMPT = """你是一名中西医结合临床望诊专家，专注于眼部及眼周异常与消化系统、神经系统病变的关联判断，并重视幽门螺杆菌等微生物病原体的循证验证。

【输入格式】
每条样本包含以下字段：
- `image_caption`：图片编号（如"图8-3"）
- `section_title`：病例标题
...
5. 若无明确命中，则生成一套以"眼周/眼睑"为部位的默认题组
...
Q4（gold_standard_labs）：
  问题：辅助检查中有哪些微生物检测、内镜或影像学结果？幽门螺杆菌检测结果如何？
  答案：精确提取以下信息（如有，含数值和单位）：
    - 幽门螺杆菌（HP）：检测阳性程度（+/++/+++）及检测方式（C13呼气试验/胃镜取样/快速尿素酶试验）
    - 胃镜：炎症类型（慢性浅表性/萎缩性/糜烂性）、伴发疾病（霉菌性食管炎/息肉/反流性食管炎）
    - 腹部超声：胆囊壁改变（胆固醇结晶、增厚mm）、胆总管变化（扩张mm）、肝实质回声
    - 面神经评估：神经系统查体阳性体征（面瘫分级 House-Brackmann I-VI 级、额纹、鼻唇沟、眼裂）
    - 其他微生物：真菌培养结果、幽门螺杆菌培养及药敏

Q5（chain_of_thought_mapping）：
  问题：医生是如何将[命中部位]的空间位置与最终诊断联系起来的？
  答案：引用"分析"字段，说明眼部/眼周体征区域与消化系统、神经系统的中医投射对应关系；
...
[
  [
    ...
    {
      "question": "辅助检查中有哪些微生物检测、内镜或影像学结果？幽门螺杆菌检测结果如何？",
      "answer": "（微生物/内镜/影像检查结果，含HP阳性程度）",
      "type": "gold_standard_labs",
      "hit_region": "[命中部位]"
    },
    {
      "question": "医生是如何将[命中部位]的空间位置与最终诊断联系起来的？",
      "answer": "（体表投射区与脏腑/神经系统对应关系分析）",
...
【硬性约束】
- HP 阳性程度（+/++/+++）必须与 context_text 原文一致，不得自行判断
- 胃镜分级、面瘫分级等数值必须与原文完全一致
...
# ── 核心处理器 ────────────────────────────────────────────────────────────────
class ZhongxiCh8VQAProcessor(ShezhenVQAProcessor):
    """《中西医结合望诊启迪》第8章 眼部异常 VQA 处理器（微生物/病原体扩容版）。"""
    ...
    def _load_samples(
        ...
        """加载 split_samples.jsonl，仅保留第8章样本（figure_id 前缀为 '8-'）。"""
        import random
    ...
    def run(
        ...
        conversations = [
            [
                {"role": "system", "content": ZHONGXI_CH8_PROMPT},
                {"role": "user", "content": u},
            ]
        ...
        with debug_file.open("w", encoding="utf-8") as dbg, out_file.open(
            ...
            for idx, (sample, resp) in enumerate(
                ...
                for group in qa_groups:
                    ...
                    hit_region = str(group[0].get("hit_region", "眼周/眼睑")).strip()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=f"《中西医结合望诊启迪》第{CHAPTER}章 眼部异常 雷达扫描裂变 VQA 生成脚本"
    )
    parser.add_argument(
        "--input-dir",
        type=str,
    ...
    processor = ZhongxiCh8VQAProcessor(
        input_dir=input_dir,
        output_dir=output_dir,
