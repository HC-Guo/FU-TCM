"""
《中西医结合望诊启迪》第5章 颜面异常 VQA 生成脚本。

核心机制：雷达扫描裂变（Radar-Scan-Fission）
  - 读取 context_text，识别命中部位
  - 候选池：整体面色/面容、两颊/颧骨、额部/颞部、眼周/眼睑、口周/唇部、舌体、静脉血管
  - 对每个命中部位独立生成 5 题 CoT 题组
  - 即：1 病例图 × N 命中部位 = N 套独立题组

输入：
  maizhen_vqa_workdir/output/manual_split/中西医结合望诊启迪/split_samples.jsonl
  （仅处理 figure_id 前缀为 "5-" 的条目）

输出（临时中间文件）：
  maizhen_vqa_workdir/output/zhongxi_vqa/ch5/vqa_dataset.jsonl
  maizhen_vqa_workdir/output/zhongxi_vqa/ch5/raw_debug.txt

最终合并后写入：
  maizhen_vqa_workdir/output/unified_output/中西医结合望诊启迪/vqa_dataset.jsonl
  （由 merge_zhongxi_vqa.py 完成合并，格式与 unified_output 其他书目一致）
"""
...
DEFAULT_OUTPUT = WORK_DIR / "output" / "zhongxi_vqa" / "ch5"
BOOK_NAME = "中西医结合望诊启迪"
CHAPTER = 5

# ── 第5章 系统提示词 ──────────────────────────────────────────────────────────
ZHONGXI_CH5_PROMPT = """你是一名中西医结合临床望诊专家，擅长从面部异常体征推断内脏病理并通过现代检查加以验证。

【输入格式】
每条样本包含以下字段：
- `image_caption`：图片编号（如"图5-3"）
- `section_title`：病例标题（如"病例1：面色萎黄——慢性胃炎"）
...
5. 若无明确命中部位，则生成一套以"整体面色/面容"为部位的默认题组
...
Q1（visual_feature）：
  ...
  答案：根据 context_text 中的"中医望诊"内容，描述该部位的客观视觉特征

Q2（clinical_tcm）：
  问题：结合既往史，该体征提示了什么中医病理机制或脏腑异常？
  答案：结合"既往史"和中医理论，说明中医病理机制

Q3（clinical_diagnosis）：
  问题：结合该体征与患者主诉，最终的临床确诊疾病是什么？
  答案：给出"诊断"字段的结论

Q4（gold_standard_tests）：
  问题：临床通过哪些现代仪器或生化检查印证了该诊断？结果如何？
  答案：列出"辅助检查"字段的所有检查项目和结果（含数值和单位）

Q5（chain_of_thought_mapping）：
  问题：医生是如何将[命中部位]的空间位置与最终诊断联系起来的？
  答案：引用"分析"字段中的体表投射区与内脏对应关系说明；若无"分析"字段，从其他字段推理作答并注明"文本未提供明确分析"

【输出格式】
- 若有 N 个命中部位，输出一个包含 N 个数组的外层数组
- 每个内层数组对应一个命中部位，包含 5 个题目对象
  ...
  [
    {
      ...
      "answer": "（根据context_text的中医望诊内容描述）",
      "type": "visual_feature",
      "hit_region": "[命中部位]"
    },
    {
...
- 每条 Q4 必须列出所有辅助检查指标，包含具体数值（如 ALT 52 U/L）
...
def _zhongxi_chapter(figure_id: str) -> int:
    """从 figure_id（如 '5-3'）提取章节号，返回 5-10 或 0（跳过）。"""
...
def _extract_multi_qa_groups(text: str) -> list[list[dict]]:
    """
    从 LLM 响应中解析多套题组（外层数组包含多个内层数组）。
    兼容以下格式：
      1. [[{...},{...},...], [{...},...]]   ← 标准多组
      2. [{...},{...},...]                  ← 单组（无外层包裹）
      3. {"qa_pairs": [...]}               ← 旧格式兼容
    """
    # 清除 think 标签和 markdown 包裹
...
# ── 核心处理器 ────────────────────────────────────────────────────────────────
class ZhongxiCh5VQAProcessor(ShezhenVQAProcessor):
    """《中西医结合望诊启迪》第5章 颜面异常 VQA 处理器。"""
    ...
    def _load_samples(
        ...
        """加载 split_samples.jsonl，仅保留第5章样本（figure_id 前缀为 '5-'）。"""
        import random
    ...
    def _build_user_prompt(self, sample: dict) -> str:
        """构建用户侧输入，包含 image_caption、section_title 和 context_text。"""
        return json.dumps(
            {
    ...
    def run(
        ...
        conversations = [
            [
                {"role": "system", "content": ZHONGXI_CH5_PROMPT},
                {"role": "user", "content": u},
            ]
        ...
        with debug_file.open("w", encoding="utf-8") as dbg, out_file.open(
            ...
            for idx, (sample, resp) in enumerate(
                ...
                for group in qa_groups:
                    ...
                    # 获取该组的 hit_region（从第一条 QA 读取）
                    hit_region = str(group[0].get("hit_region", "整体面色/面容")).strip()
                    # 生成安全的 ID 后缀
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=f"《中西医结合望诊启迪》第{CHAPTER}章 颜面异常 雷达扫描裂变 VQA 生成脚本"
    )
    parser.add_argument(
        "--input-dir",
        type=str,
    ...
    processor = ZhongxiCh5VQAProcessor(
        input_dir=input_dir,
        output_dir=output_dir,
