"""
《中西医结合望诊启迪》第9章 舌部异常 VQA 生成脚本。

核心机制：雷达扫描裂变（Radar-Scan-Fission）
  - 读取 context_text，识别命中部位
  - 候选池：整体舌体、舌苔、舌中/舌前、面色/额部/鼻柱
  - 对每个命中部位独立生成 5 题 CoT 题组
  - Q4 专属：内镜分期/病理活检/叶酸/维B12等病理与微量元素指标
  - Q3 补充：药源性和既往史诱因评估
  - Q5 补充：舌诊部位与脏腑的经典映射关系

输入：
  maizhen_vqa_workdir/output/manual_split/中西医结合望诊启迪/split_samples.jsonl
  （仅处理 figure_id 前缀为 "9-" 的条目）

输出：
  maizhen_vqa_workdir/output/zhongxi_vqa/ch9/vqa_dataset.jsonl
  maizhen_vqa_workdir/output/zhongxi_vqa/ch9/raw_debug.txt
"""
...
DEFAULT_OUTPUT = WORK_DIR / "output" / "zhongxi_vqa" / "ch9"
BOOK_NAME = "中西医结合望诊启迪"
CHAPTER = 9

# ── 第9章 系统提示词（病理与微量元素升级版）────────────────────────────────
ZHONGXI_CH9_PROMPT = """你是一名中西医结合临床望诊专家，专注于舌部异常与消化系统、营养代谢病变的关联判断，重视病理活检和微量元素指标的循证验证。

【输入格式】
每条样本包含以下字段：
- `image_caption`：图片编号（如"图9-3"）
- `section_title`：病例标题
...
Q1（visual_feature）：
  ...
  答案：根据 context_text 中"中医望诊"内容，描述该部位的客观视觉特征（舌色、苔色、苔质、舌体形态等）

Q2（clinical_tcm）：
  问题：结合既往史，该体征提示了什么中医病理机制或脏腑异常？
  答案：结合"既往史"与中医理论，说明中医病理机制

Q3（clinical_diagnosis）：
  问题：结合该体征与患者主诉，最终的临床确诊疾病是什么？
  答案：给出"诊断"字段的完整结论
  补充规则：若 context_text 提及既往用药史或特殊病史，应同时评估该因素对消化道黏膜的影响

Q4（gold_standard_labs_pathology）：
  问题：辅助检查中涉及哪些内镜分期、病理活检或营养微量元素指标？具体结果如何？
  答案：精确提取以下信息（如有，含数值和单位）：
    - 胃镜内镜分期：溃疡活动期（A1/A2期）、愈合期（H1/H2期）、瘢痕期（S1/S2期）
    - 内镜下病理表现：肠上皮化生（轻度/中度/重度）、固有腺减少程度、腺上皮增生活跃
    - 病理活检结论：浅表黏膜慢性炎（轻/中/重度）、平滑肌增生、黏膜肌层改变
    - 营养指标：血叶酸（ng/L，正常值>7.0 ng/L）、维生素B12（pg/ml，正常值>200 pg/ml）
    - 微生物：HP 检测结果（+/-/++/+++）及检测方式

Q5（chain_of_thought_mapping）：
  问题：医生是如何将[命中部位]的空间位置与最终诊断联系起来的？
  答案：运用舌诊部位映射规则（舌中前→脾胃，舌根→肾，等），引用"分析"字段中说明体征与诊断之间的逻辑链；
...
[
  [
    ...
    },
    {
      ...
      "answer": "（诊断结论，含药源性/既往史影响评估）",
      "type": "clinical_diagnosis",
      "hit_region": "[命中部位]"
    },
    {
      "question": "辅助检查中涉及哪些内镜分期、病理活检或营养微量元素指标？具体结果如何？",
      "answer": "（内镜分期/病理活检/叶酸/维B12等，含数值和单位）",
      "type": "gold_standard_labs_pathology",
      "hit_region": "[命中部位]"
    },
    {
      "question": "医生是如何将[命中部位]的空间位置与最终诊断联系起来的？",
      "answer": "（舌诊部位映射推理链，引用分析字段）",
...
【硬性约束】
- 叶酸、维生素B12数值必须与 context_text 原文完全一致，包含单位
- 内镜分期代码（A1/A2/H1/H2/S1/S2）必须与原文一致
...
# ── 核心处理器 ────────────────────────────────────────────────────────────────
class ZhongxiCh9VQAProcessor(ShezhenVQAProcessor):
    """《中西医结合望诊启迪》第9章 舌部异常 VQA 处理器（病理与微量元素升级版）。"""
    ...
    def _load_samples(
        ...
        """加载 split_samples.jsonl，仅保留第9章样本（figure_id 前缀为 '9-'）。"""
        import random
    ...
    def run(
        ...
        conversations = [
            [
                {"role": "system", "content": ZHONGXI_CH9_PROMPT},
                {"role": "user", "content": u},
            ]
        ...
        with debug_file.open("w", encoding="utf-8") as dbg, out_file.open(
            ...
            for idx, (sample, resp) in enumerate(
                ...
                for group in qa_groups:
                    ...
                    hit_region = str(group[0].get("hit_region", "整体舌体")).strip()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=f"《中西医结合望诊启迪》第{CHAPTER}章 舌部异常 雷达扫描裂变 VQA 生成脚本"
    )
    parser.add_argument(
        "--input-dir",
        type=str,
    ...
    processor = ZhongxiCh9VQAProcessor(
        input_dir=input_dir,
        output_dir=output_dir,
