"""
《中西医结合望诊启迪》第10章 额部异常 VQA 生成脚本。

核心机制：雷达扫描裂变（Radar-Scan-Fission）
  - 读取 context_text，识别命中部位
  - 候选池：额部、鼻部、舌体、眼部、口部
  - 对每个命中部位独立生成 5 题 CoT 题组
  - Q4 专属：同型半胱氨酸/13C呼气试验/颈血管超声等分子生化指标
  - Q3 补充：拒绝检查情况记录
  - Q5 强调：经络循行理论（足阳明胃经/足少阳胆经等）

输入：
  maizhen_vqa_workdir/output/manual_split/中西医结合望诊启迪/split_samples.jsonl
  （仅处理 figure_id 前缀为 "10-" 的条目）

输出：
  maizhen_vqa_workdir/output/zhongxi_vqa/ch10/vqa_dataset.jsonl
  maizhen_vqa_workdir/output/zhongxi_vqa/ch10/raw_debug.txt
"""
...
DEFAULT_OUTPUT = WORK_DIR / "output" / "zhongxi_vqa" / "ch10"
BOOK_NAME = "中西医结合望诊启迪"
CHAPTER = 10

# ── 第10章 系统提示词（经络循行版）──────────────────────────────────────────
ZHONGXI_CH10_PROMPT = """你是一名中西医结合临床望诊专家，专注于额部异常与消化系统、神经代谢病变的关联判断，尤其重视经络理论在体表定位诊断中的应用。

【输入格式】
每条样本包含以下字段：
- `image_caption`：图片编号（如"图10-3"）
- `section_title`：病例标题
...
Q2（clinical_tcm）：
  ...
  答案：结合"既往史"与中医理论，说明中医病理机制（优先引用经络循行理论）

Q3（clinical_diagnosis）：
  问题：结合该体征与患者主诉，最终的临床确诊疾病是什么？
  答案：给出"诊断"字段的完整结论
  拒绝检查处理规则：若 context_text 中出现以下表述，Q3 需专门记录：
    - "患者不能耐受" → 格式："根据望诊推断[疾病]，建议[检查]，但患者不能耐受，仅依据已有证据诊断为[最终诊断]。"
    - "未予采纳" → 格式："根据望诊推断[疾病]，建议[检查]，但患者未予采纳，仅依据已有证据诊断为[最终诊断]。"
    - "患者未查" → 格式："根据望诊推断[疾病]，建议[检查]，但患者未查，仅依据已有证据诊断为[最终诊断]。"

Q4（gold_standard_labs_molecular）：
  问题：辅助检查中涉及哪些分子生化指标或特殊检查？如同型半胱氨酸、C13呼气试验等，结果如何？
  答案：精确提取以下信息（如有，含数值和单位）：
    - 同型半胱氨酸（Hcy）：数值（μmol/L），正常值<15 μmol/L，升高提示动脉硬化风险
    - 13C-尿素呼气试验：DOB 值（‰），HP 阳性判断阈值（>4.0‰）
    - 血叶酸（ng/L，正常值>7.0 ng/L）
    - 维生素B12（pg/ml，正常值>200 pg/ml）
    - 颈血管超声：颈动脉内中膜厚度（IMT，mm）、斑块存在与否
    - 其他生化：ALT（U/L）、血脂（TG/TC，mmol/L）、血糖（GLU，mmol/L）

Q5（chain_of_thought_mapping）：
  问题：医生是如何将[命中部位]的空间位置与最终诊断联系起来的？
  答案：运用经络循行理论（额部→足阳明胃经/足少阳胆经等），引用"分析"字段中说明体征与诊断之间的逻辑链；
        模板："[命中部位]位于[经络名称]循行区域，中医认为该经络与[脏腑]功能相关。望诊发现[体征]，提示[脏腑]功能[异常表现]，与最终确诊的[疾病]在[检查结果]上相互印证。"
...
[
  [
    ...
    },
    {
      ...
      "answer": "（经络理论+中医病理机制说明）",
      "type": "clinical_tcm",
      "hit_region": "[命中部位]"
    },
    {
      "question": "结合该体征与患者主诉，最终的临床确诊疾病是什么？",
      "answer": "（诊断结论，含拒绝检查情况记录）",
      "type": "clinical_diagnosis",
      "hit_region": "[命中部位]"
    },
    {
      "question": "辅助检查中涉及哪些分子生化指标或特殊检查？如同型半胱氨酸、C13呼气试验等，结果如何？",
      "answer": "（Hcy/13C呼气/叶酸/维B12/颈血管超声等，含数值和单位）",
      "type": "gold_standard_labs_molecular",
      "hit_region": "[命中部位]"
    },
    {
      "question": "医生是如何将[命中部位]的空间位置与最终诊断联系起来的？",
      "answer": "（经络循行理论推理链，引用分析字段）",
...
【硬性约束】
- Hcy 数值、DOB 值、IMT 厚度等必须与 context_text 原文完全一致，包含单位
- 若某检查 context_text 未提及，直接跳过，不得补充
...
# ── 核心处理器 ────────────────────────────────────────────────────────────────
class ZhongxiCh10VQAProcessor(ShezhenVQAProcessor):
    """《中西医结合望诊启迪》第10章 额部异常 VQA 处理器（经络循行版）。"""
    ...
    def _load_samples(
        ...
        """加载 split_samples.jsonl，仅保留第10章样本（figure_id 前缀为 '10-'）。"""
        import random
    ...
    def run(
        ...
        conversations = [
            [
                {"role": "system", "content": ZHONGXI_CH10_PROMPT},
                {"role": "user", "content": u},
            ]
        ...
        with debug_file.open("w", encoding="utf-8") as dbg, out_file.open(
            ...
            for idx, (sample, resp) in enumerate(
                ...
                for group in qa_groups:
                    ...
                    hit_region = str(group[0].get("hit_region", "额部")).strip()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=f"《中西医结合望诊启迪》第{CHAPTER}章 额部异常 雷达扫描裂变 VQA 生成脚本"
    )
    parser.add_argument(
        "--input-dir",
        type=str,
    ...
    processor = ZhongxiCh10VQAProcessor(
        input_dir=input_dir,
        output_dir=output_dir,
